    ...
    "vendor/topbar.js"(exports, module) {
      (function (window2, document2) {
        ...
        (function () {
          ...
          for (
            var x = 0;
            x < vendors.length && !window2.requestAnimationFrame;
            ++x
          ) {
            window2.requestAnimationFrame =
              window2[vendors[x] + "RequestAnimationFrame"];
            window2.cancelAnimationFrame =
              window2[vendors[x] + "CancelAnimationFrame"] ||
              window2[vendors[x] + "CancelRequestAnimationFrame"];
          }
  ...
  var DOM = {
    ...
    maintainPrivateHooks(fromEl, toEl, phxViewportTop, phxViewportBottom) {
      if (
        ...
        toEl.setAttribute(
          "data-phx-hook",
          fromEl.getAttribute("data-phx-hook"),
        );
      }
      if (
        toEl.hasAttribute &&
    ...
    isUsedInput(el) {
      return (
        ...
        (this.private(el, PHX_HAS_FOCUSED) ||
          this.private(el, PHX_HAS_SUBMITTED))
      );
    },
    resetForm(form) {
...
`);
      ...
      mounted() {
        ...
        this.focusStart.addEventListener("focus", (e) => {
          if (!e.relatedTarget || !this.el.contains(e.relatedTarget)) {
            ...
            aria_default.attemptFocus(nextFocus) ||
              aria_default.focusFirst(nextFocus);
          } else {
        ...
        this.focusEnd.addEventListener("focus", (e) => {
          if (!e.relatedTarget || !this.el.contains(e.relatedTarget)) {
            ...
            aria_default.attemptFocus(nextFocus) ||
              aria_default.focusLast(nextFocus);
          } else {
  ...
  var DOMPatch = class {
    ...
    perform(isJoinPatch) {
      ...
      function morph(
        ...
        let morphCallbacks = {
          ...
          addChild: (parent, child) => {
            ...
            if (streamAt === 0) {
              ...
              if (lastChild && !lastChild.hasAttribute(PHX_STREAM_REF)) {
                let nonStreamChild = Array.from(parent.children).find(
          ...
          onBeforeElUpdated: (fromEl, toEl) => {
            if (
              fromEl.id &&
              fromEl.isSameNode(targetContainer2) &&
              fromEl.id !== toEl.id
            ) {
            ...
            if (fromEl.hasAttribute(PHX_REF_SRC)) {
              ...
              if (
                ...
                let clone2 = isLocked
                  ? dom_default.private(fromEl, PHX_REF_LOCK) ||
      ...
      if (externalFormTriggered) {
        liveSocket2.unload();
        const submitter = dom_default.private(
        ...
        if (
          submitter &&
          submitter.name &&
          targetContainer.contains(submitter)
        ) {
  ...
  var JS = {
    ...
    exec_focus_first(e, eventType, phxEvent, view, sourceEl, el) {
      ...
      window.requestAnimationFrame(() => {
        window.requestAnimationFrame(
          () =>
            aria_default.focusFirstInteractive(el) ||
            aria_default.focusFirst(el),
        );
      });
    },
    ...
    filterToEls(liveSocket2, sourceEl, { to }) {
      ...
      return to
        ? liveSocket2.jsQuerySelectorAll(sourceEl, to, defaultQuery)
        : [sourceEl];
    },
    defaultDisplay(el) {
      return (
    ...
    pushEvent(event, payload = {}, onReply) {
      if (onReply === void 0) {
        return new Promise((resolve, reject) => {
          try {
            const ref = this.__view().pushHookEvent(
              this.el,
              null,
              event,
              payload,
              (reply, _ref) => resolve(reply),
            );
            if (ref === false) {
              reject(
      ...
      return this.__view().pushHookEvent(
        this.el,
        null,
        event,
        payload,
  ...
  var serializeForm = (form, opts, onlyNames = []) => {
    ...
    const { inputsUnused, onlyHiddenInputs } = Array.from(form.elements).reduce(
      (acc, input) => {
        const {
          inputsUnused: inputsUnused2,
          onlyHiddenInputs: onlyHiddenInputs2,
        } = acc;
  ...
  var View = class _View {
    ...
    getDescendentByEl(el) {
      ...
      if (el.id === this.id) {
        ...
        return (_a = this.children[el.getAttribute(PHX_PARENT_ID)]) == null
          ? void 0
          : _a[el.id];
      }
    }
    ...
    addHook(el) {
      ...
      if (hookElId && !this.viewHooks[hookElId]) {
        let hook =
          dom_default.getCustomElHook(el) ||
    ...
    onLiveRedirect(redir) {
      ...
      let e = new CustomEvent("phx:server-navigate", {
    ...
    undoRefs(ref, phxEvent, onlyEls) {
      ...
      if (onlyEls) {
        ...
        dom_default.all(document, selector, (parent) => {
          ...
          dom_default.all(parent, selector, (child) =>
            this.undoElRef(child, ref, phxEvent),
          );
          this.undoElRef(parent, ref, phxEvent);
        });
      } else {
        dom_default.all(document, selector, (el) =>
          this.undoElRef(el, ref, phxEvent),
        );
      }
    }
    undoElRef(el, ref, phxEvent) {
      ...
      elRef.maybeUndo(ref, phxEvent, (clonedTree) => {
        let patch = new DOMPatch(this, el, this.id, clonedTree, [], null, {
          undoRef: ref,
        });
        const phxChildrenAdded = this.performPatch(patch, true);
        dom_default.all(el, `[${PHX_REF_SRC}="${this.refSrc()}"]`, (child) =>
          this.undoElRef(child, ref, phxEvent),
        );
        if (phxChildrenAdded) {
          this.joinNewChildren();
      ...
      for (let { el, lock, loading } of elements) {
        ...
        let detail = {
          ...
          lockElements: elements
            .filter(({ lock: lock2 }) => lock2)
            .map(({ el: el2 }) => el2),
          loadingElements: elements
            .filter(({ loading: loading2 }) => loading2)
            .map(({ el: el2 }) => el2),
          unlock: (els) => {
    ...
    disableForm(formEl, phxEvent, opts = {}) {
      ...
      let formEls = disables
        .concat(buttons)
        .concat(inputs)
        .map((el) => {
          return { el, loading: true, lock: true };
        });
      let els = [{ el: formEl, loading: true, lock: false }]
        .concat(formEls)
        .reverse();
    ...
    pushLinkPatch(e, href, targetEl, callback) {
      ...
      let refGen = targetEl
        ? () =>
            this.putRef([{ el: targetEl, loading, lock: true }], null, "click")
        : null;
  ...
  var LiveSocket = class {
    constructor(url, phxSocket, opts = {}) {
      ...
      this.disconnectedTimeout =
        opts.disconnectedTimeout || DISCONNECTED_TIMEOUT;
    ...
    execJS(el, encodedJS, eventType = null) {
      ...
      this.owner(el, (view) =>
        js_default.exec(e, eventType, encodedJS, view, el),
      );
    }
    // private
    execJSHookPush(el, phxEvent, data, callback) {
      this.withinOwners(el, (view) => {
        let e = new CustomEvent("phx:exec", { detail: { sourceElement: el } });
        js_default.exec(e, "hook", phxEvent, view, el, [
          "push",
          { data, callback },
        ]);
      });
    ...
    bindTopLevelEvents({ dead } = {}) {
      ...
      this.bind(
        ...
        (e, type, view, targetEl, phxEvent, _phxTarget) => {
          ...
          js_default.exec(e, type, phxEvent, view, targetEl, [
            "push",
            { data },
          ]);
        },
    ...
    historyPatch(href, linkState, linkRef = this.setPendingLink(href)) {
      ...
      browser_default.updateCurrentState((state) =>
        __spreadProps(__spreadValues({}, state), { backType: "patch" }),
      );
